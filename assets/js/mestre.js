...
import { db, ref, onValue, update, set, remove, SALA_ID } from './firebase-init.js';
...
onValue(salaRef, (snap) => {
  if (snap.exists()) {
    ...
    } else if (data.status !== 'jogando' && !jogoFinalizado && window.location.pathname.includes('mestre.html')) {
...
onValue(jogadoresRef, (snap) => {
  ...
  for (const jog of novosJogadores) {
    ...
    if (antigo && antigo.presente === true && jog.presente === false && !jogoFinalizado) {
  ...
  jogadoresAnteriores = {};
  jogadoresAtivos.forEach(j => { jogadoresAnteriores[j.nome] = { presente: j.presente }; });
...
window.onload = async () => {
  ...
  if (!salaSnap.exists() || salaSnap.val().status !== 'jogando') {
    alert("O jogo não foi iniciado ou já foi finalizado. Redirecionando para o lobby.");
    window.location.href = "lobby.html?role=mestre";
    return;
  }
  ...
  try {
    ...
    if (perguntasRestantes.length === 0) {
      ...
      for (let i = perguntasRestantes.length - 1; i > 0; i--) {
        ...
        [perguntasRestantes[i], perguntasRestantes[j]] = [perguntasRestantes[j], perguntasRestantes[i]];
      }
      localStorage.setItem(`perguntas_${SALA_ID}`, JSON.stringify(perguntasRestantes));
    }
...
function renderizarPainelMestre() {
  ...
  document.getElementById('btnFinalizar')?.addEventListener('click', finalizarJogo);
}

function carregarListaJogadoresHTML() {
  if (!jogadoresAtivos.length) return '<p class="text-muted p-3">Nenhum jogador.</p>';
  const ordenados = [...jogadoresAtivos].sort((a,b) => {
    if (a.presente !== b.presente) return a.presente ? -1 : 1;
    return (b.pontuacao||0) - (a.pontuacao||0);
  });
  return `<ul class="list-group list-group-flush">${ordenados.map(j => `
...
function atualizarListaJogadoresMestre() {
  ...
  if (container && !jogoFinalizado) container.innerHTML = carregarListaJogadoresHTML();
...
async function sortearPergunta() {
  ...
  if (restantesSpan) restantesSpan.textContent = `📋 Perguntas: ${perguntasRestantes.length}`;
...
async function finalizarJogo() {
  ...
  if (confirm("Deseja realmente finalizar o jogo? Todos os dados da partida atual serão perdidos.")) {
